package com.tmstudios.paperboard.audio;
...
public class MicrophoneEncoder implements Runnable {
	...
	private EncoderThread mThread=null;

    public MicrophoneEncoder(SessionConfig config) throws IOException {
        init(config,null);
    }
	public MicrophoneEncoder(SessionConfig config,EncoderThread thread) throws IOException{
		init(config,thread);
	}

	public void setThread(EncoderThread mThread)
	{
		this.mThread = mThread;
	}

    private void init(SessionConfig config,EncoderThread thread) throws IOException {
