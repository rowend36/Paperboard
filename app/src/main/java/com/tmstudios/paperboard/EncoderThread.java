...
import com.tmstudios.paperboard.audio.*;
	...
	private static final int MSG_AUDIO_AVAILABLE = 3;
		 ...
		 */
		public EncoderThread(VideoEncoderCore videoEncoder,AudioEncoderCore audioEncoder,AndroidMuxer muxer) {
		...
		public void audioAvailableSoon(){
			...
			Log.e(TAG,"audio");
			mHandler.sendMessage(mHandler.obtainMessage(MSG_AUDIO_AVAILABLE));
		}
		...
		private static class EncoderHandler extends Handler {
			private WeakReference<EncoderThread> mWeakEncoder;

			public EncoderHandler(EncoderThread encoder) {
				mWeakEncoder = new WeakReference<EncoderThread>(encoder);
			}
			...
			public void handleMessage(Message inputMessage) {
				...
				EncoderThread encoder = mWeakEncoder.get();
				if (encoder == null) {
				...
				switch (what) {
					...
					case MSG_FRAME_AVAILABLE:
						...
						break;
					case MSG_AUDIO_AVAILABLE:
						encoder.handleAudioAvailable();
						break;
					default:
		...
		private void handleAudioAvailable() {
			if (VERBOSE) Log.d(TAG, "handleAudioAvailable");
